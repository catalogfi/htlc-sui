...
contract HTLC is EIP712 {
    ...
    error SameInitiatorAndRedeemer();
    error DuplicateOrder();
    error InvalidRedeemerSignature();
    ...
    modifier safeParams(address redeemer, uint256 timelock, uint256 amount) {
        require(redeemer != address(0), ZeroAddressRedeemer());
        require(timelock > 0, ZeroTimelock());
    ...
    function redeem(bytes32 orderID, bytes calldata secret) external {
        ...
        require(order.redeemer != address(0x0), OrderNotInitiated());
        require(!order.isFulfilled, OrderFulfilled());

        bytes32 secretHash = sha256(secret);

        require(sha256(abi.encode(block.chainid, secretHash, order.initiator)) == orderID, IncorrectSecret());
    ...
    function refund(bytes32 orderID) external {
        ...
        require(order.redeemer != address(0), OrderNotInitiated());
        require(!order.isFulfilled, OrderFulfilled());
        require(order.initiatedAt + order.timelock < block.number, OrderNotExpired());
    ...
    function _initiate(
        ...
        require(initiator_ != redeemer_, SameInitiatorAndRedeemer());
        
        bytes32 orderID = sha256(abi.encode(block.chainid, secretHash_, initiator_));
        Order memory order = orders[orderID];

        require(order.redeemer == address(0), DuplicateOrder());
    ...
    function instantRefund(bytes32 orderID, bytes calldata signature) external {
        ...
        require(order.redeemer == redeemer, InvalidRedeemerSignature());
        require(!order.isFulfilled, OrderFulfilled());
